...
`vinhos.imagem_url` é uma coluna **nova** — a app deteta se ela existe
(`detetarImagem()`) e, enquanto não existir na BD, esconde o campo e não o
manda nas gravações. Sem isso, um PATCH a uma base ainda sem a coluna
rebentava **todas** as gravações com 400. O `ALTER TABLE` está em
`db/schema.sql` e em `db/README.md`.
